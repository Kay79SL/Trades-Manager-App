...
VAT_RATE = 0.23  # Irish VAT on most trades
...
def estimate_materials(job_name: str) -> dict[str, Any]:
    ...
    recipe = graph_search.items_for_job_by_name(job_name)
    ...
    subtotal = 0.0
    for entry in recipe:
        qty = entry.get("typical_quantity") or 1
    ...
    return {
        ...
        "source":         "neo4j_recipe + mongo_prices",
    }
...
def estimate_labour(job_type_id: str | None, job_name: str | None) -> dict[str, Any]:
    ...
    for inv in invoices:
        cost = inv.get("labour_cost_ex_vat") or inv.get("labour_cost") or 0
        if cost > 0:
    ...
    if not labour_costs:
        return {
            ...
            "max_eur":    0.0,
            "n_invoices": 0,
            "source":     "no_past_invoices",
        }

    return {
        "median_eur": round(median(labour_costs), 2),
...
def benchmark_against_pos(job_name: str) -> dict[str, Any]:
    ...
    """
    pos = graph_search.similar_pos_by_job_type(job_name)
    if not pos:
        return {
            "n_pos":          0,
            "avg_subtotal":   0.0,
    ...
    subtotals = [p["subtotal"] for p in pos if p.get("subtotal")]
    totals = [p["total"] for p in pos if p.get("total")]
...
def compute_confidence(materials: dict, labour: dict, benchmark: dict) -> str:
    ...
    """
    has_recipe = materials.get("n_recipe_items", 0) > 0
    has_labour = labour.get("n_invoices", 0) >= 3
    has_benchmark = benchmark.get("n_pos", 0) > 0

    score = sum([has_recipe, has_labour, has_benchmark])
    ...
    if has_recipe and has_benchmark and benchmark.get("avg_subtotal", 0) > 0:
        ...
        diff_pct = abs(predicted_subtotal - benchmark_subtotal) / benchmark_subtotal
        if diff_pct < 0.20 and score == 3:
            return "high"
        elif diff_pct < 0.35 and score >= 2:
            return "medium"
        else:
            return "low"
...
def predict_quote(
    ...
    """
    materials = estimate_materials(job_name)
    labour = estimate_labour(job_type_id, job_name)
    benchmark = benchmark_against_pos(job_name)

    subtotal_ex_vat = materials["subtotal"] + labour["median_eur"]
    vat = round(subtotal_ex_vat * VAT_RATE, 2)
    total_inc_vat = round(subtotal_ex_vat + vat, 2)

    confidence = compute_confidence(materials, labour, benchmark)
    ...
    return {
        ...
        "evidence": {
            ...
            "stores_used":         ["mongo_invoices", "mongo_items", "neo4j_recipe", "neo4j_pos"],
        },
    }
...
if __name__ == "__main__":
    import sys, json
    job = sys.argv[1] if len(sys.argv) > 1 else "Boiler installation"
